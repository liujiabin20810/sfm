...
namespace bundler
{
	...
	int BundleAdjuster::Count2DMeasurements(const vector<CloudPoint>& pointcloud) {
		...
		for (unsigned int i=0; i<pointcloud.size(); i++) {
			for (unsigned int ii=0; ii<pointcloud[i].pt2d_index_of_img.size(); ii++) {
				if (pointcloud[i].pt2d_index_of_img[ii] > 0) {
					K ++;
				}
			}
		}
	...
	int BundleAdjuster::Count2DMeasurements(const vector<PointData>& pointcloud)
	{
		int K = 0;
		for (unsigned int i=0; i<pointcloud.size(); i++) 
		{
			K += pointcloud[i].m_num_vis;
		}
		return K;
	}
									...
									) 
	{
		...
		{
			int pointId = j;
			Xs[j][0] = pointcloud[j].pt3d.x;
			Xs[j][1] = pointcloud[j].pt3d.y;
			Xs[j][2] = pointcloud[j].pt3d.z;
		...
		{
			for (unsigned int i=0; i<pointcloud[k].pt2d_index_of_img.size(); i++) {
				if (pointcloud[k].pt2d_index_of_img[i] >= 0) {
					int view = i, point = k;
					Vector3d p, np;
				
					Point cvp = imgpts[i][pointcloud[k].pt2d_index_of_img[i]].pt;
					p[0] = cvp.x;
					p[1] = cvp.y;
					p[2] = 1.0;
	...
	//		StdMetricBundleOptimizer opt(inlierThreshold,cams,Xs,measurements,correspondingView,correspondingPoint);
	...
	
		{
			...
			for (unsigned int j = 0; j < Xs.size(); ++j)
			{
				...
				pointcloud[j].pt3d.x = Xs[j][0];
				pointcloud[j].pt3d.y = Xs[j][1];
				pointcloud[j].pt3d.z = Xs[j][2];
			}
	...
	void BundleAdjuster::adjustBundle(std::vector<PointData>& pointcloud, 
						  cv::Mat& cam_matrix,
						  const std::vector<std::vector<cv::KeyPoint> >& imgpts,
						  std::map<int ,cv::Matx34d>& Pmats)
	{
		...
		{
			int pointId = j;
			Xs[j][0] = pointcloud[j].m_pos[0];
			Xs[j][1] = pointcloud[j].m_pos[1];
			Xs[j][2] = pointcloud[j].m_pos[2];
			pointIdFwdMap[j] = pointId;
			pointIdBwdMap.insert(make_pair(pointId, j));
		}
		cout << "Read the 3D points." << endl;
		...
		for (unsigned int k = 0; k < pointcloud.size(); ++k)
		{
			for (unsigned int i=0; i<pointcloud[k].m_views.size(); i++) 
			{
				int im_idx = pointcloud[k].m_views[i].first;
				int key_idx = pointcloud[k].m_views[i].second;

				{
					int view = im_idx, point = k;
					Vector3d p, np;
				
					Point cvp = imgpts[im_idx][key_idx].pt;
					p[0] = cvp.x;
					p[1] = cvp.y;
					p[2] = 1.0;
...
//	  cout << "refined K = "; displayMatrix(K0);
		...
		scaleMatrixIP(f0, Knew);
		Knew[2][2] = 1.0;
//		std::cout << "Knew = "; displayMatrix(Knew);
...
//		showErrorStatistics(f0, distortion, cams, Xs, measurements, correspondingView, correspondingPoint);
	
		{
			...
			for (unsigned int j = 0; j < Xs.size(); ++j)
			{
				...
				pointcloud[j].m_pos[0] = Xs[j][0];
				pointcloud[j].m_pos[1] = Xs[j][1];
				pointcloud[j].m_pos[2] = Xs[j][2];
			}
